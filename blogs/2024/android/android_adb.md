...
注意：如果您在多台设备可用时发出命令但未指定目标设备，adb 会显示错误“adb: more one device/emulator”。
...
清除设备之前设置的系统 OTA 更新冻结期记录。在开发管理冻结期的应用时，这有助于避免设备存在调度方面的限制。请参阅管理系统更新。
...
按 Ctrl+C 停止屏幕录制。否则，到三分钟或 --time-limit 设置的时间限制时，录制将自动停止。
