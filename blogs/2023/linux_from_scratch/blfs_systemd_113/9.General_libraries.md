...
To test the results, issue: **PATH=$PATH:/usr/sbin ninja test**. The addition to `PATH` is needed if you use the starting scripts in [The Bash Shell Startup Files](../postlfs/profile.html "The Bash Shell Startup Files"), because some tests need to run **udevadm**. One test needs to be run in an X session.
