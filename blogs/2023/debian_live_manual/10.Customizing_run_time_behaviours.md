...
One possible way of changing the default password is by means of a hook as described in [Boot-time hooks](9.Customizing_contents.md#923-boot-time-hooks). In order to do that you can use the "passwd" hook from /usr/share/doc/live-config/examples/hooks, prefix it accordingly (e.g. 2000-passwd) and add it to config/includes.chroot/lib/live/config/
...
See also [Using the space left on a USB stick](4.The_basics.md#433-using-the-space-left-on-a-usb-stick).
...
And create the persistence.conf file in the root of the partition. This is, as explained before, strictly necessary. See [The persistence.conf file](#1031-the-persistenceconf-file).
